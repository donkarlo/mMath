#Growing Neural Gas
* Growing Neural Gas(GNG) is an incremental clustering algorithm called Growing Neural Gas (GNG) by Bernd Fritzke [[1]](#1).
* GNG works by forming a cometion between the two following phases [[2]](#1):
  * After initialization, which places two randomly generated nodes into a network, two main phases are alternating until a selected stopping crite-
rion is met. The first phase (“self-organizing”) is adaptation, which is performed in λ steps. In each step, random
...
for an identification of the area where the adaptation was least successful – the node with the largest error and its
neighbor with the largest error are found. A new node is created at the halfway between them. The errors of those
nodes are decreased.
...
### Gng Parameters
* All the default parameters can be modified while making a new GNG object
  ```
...
### Code methodology
data.cluster.gng.GNG class inherits data.cluster.gng.ClusteringStrgy class which enforces it to implement _doSetClusters method which will be used by its getClusters method  to either to compute and return back the found clusters if its the first run or just return back without re-execution of the process.
...
###Benchmarks
* The following benchmark (click here) is generated by pycharm Profiling utility while running data/cluster/gng/examples/fiveUniformShapes/trajectory/Trajectory.py
* Manual time counting using python time, for the default GNG configuration for the same file was 125.47 seconds

###Todo
* GNG:
  * GNG with Utility factor (GNG-U) implementation [[3]](#1)
  * Supervised GNG (S-GNG) implemetation 
* Graph related:
